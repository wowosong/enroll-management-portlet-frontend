      ...
      <div class="rate_hint">
        ...
        <div class="hint_info">
          ...
          <template v-if="stempInfo.regStatus != 1">
            <p class="text-left">
              您于
              <span class="color1">{{stempInfo.createTime | dateFormatYmd}}</span>完成网上报名，请在
              <span class="color1">“报名信息”</span>页面完善信息~
            </p>
            <p class="text-left">
              并按学校通知时间
              <span class="color1">到校现场确认</span>
              ，打印{{str}}卡~
            </p>
          </template>
          ...
          <template v-if="stempInfo.regStatus == 1 && stempInfo.ifEnter == null">
            <p class="text-left">
              您于
              <span class="color1">{{stempInfo.modifyTime | dateFormatYmd}}</span>
              到校办理了{{str}}卡~
            </p>
            <p class="text-left">
              请按{{str}}卡上的{{str}}时间到校
              <span class="color1">参加{{str}}</span>~
            </p>
              ...
              <template v-if="!expireFlag">
                ...
                <template v-if="!reserveObj">
                  <div class="bottomBorder">
                    <div class="text-left">
                      您于
                      <span class="color1">{{stempInfo.admitTime | dateFormatYmd}}</span>
                      被学校录取
                      <span
                        v-if="stempInfo.scholarship"
                      >，并获得本学年奖学金{{stempInfo.scholarship}}元~</span>
                    </div>
                    ...
                    <div class="over_hint text-left">
                      ...
                      <span
                        class="color1"
                      >(缴费截止时间：{{stempInfo.paymentDeadline | dateFormatYmd}})</span>
                    </div>
                  </div>
                  <!--成绩-->
                    ...
                    <table class="my-table" v-if="!isPhone">
                      <thead>
                        <tr>
                          ...
                          <th
                            v-for="(stuScore,index) in nowStuInfo.scores"
                            :key="index"
                          >{{stuScore.name}}</th>
                        </tr>
                      </thead>
                    ...
                    <div class="sign-btn">
                      ...
                      <template v-else>
                        <div class="agreement">
                          ...
                          <a
                            target="_blank"
                            href="http://openhome.cmbchina.com/pay/H5Pay/AppIntro/SceneIntro.aspx"
                          >点击查看《招行一网通介绍》</a>
                ...
                <template v-if="reserveObj">
                  <div class="text-left">
                    <i class="el-icon-circle-check" style="color: #00ff00">请在预约时间段前往学校缴费</i>
                  </div>
                  <div
                    class="text-left"
                    style="color: #ff0000"
                  >{{reserveObj.reserveDate | dateFormatYmd}}&nbsp;{{reserveObj.timeText}}</div>
                </template>
              </template>
              <!-- 逾期未缴费 -->
              <template v-if="expireFlag">
                <p class="text-left">
                  您
                  <span class="color1">逾期未缴费</span>，视为自动放弃录取资格~
                </p>
              </template>
            </div>
            <!-- 未录取 -->
          ...
          <!-- 缴费成功 -->
          <template
            v-if="stempInfo.ifPayment == 1 && stempInfo.divideClassesStatus != 1 && stempInfo.startReportInfo != 1"
          >
            <p class="bottomBorder text-left">
              您
              <span v-if="stempInfo.payTime == null">已</span>
              <span v-if="stempInfo.payTime != null">于</span>
              <span class="color1">{{stempInfo.payTime | dateFormatYmd}}</span>到校完成缴费，缴费金额
              <span class="color1">{{stempInfo.payAmount || 0}}元</span>，请及时完成
            ...
            <div class="pay_info">
              ...
              <div>
                <span>本学年奖学金：</span>
                {{stempInfo.scholarship ||0}}元
              </div>
              <div>
                <span>实际缴费：</span>
                {{stempInfo.payAmount ||0}}元
              </div>
              <template v-if="stempInfo.paymentType == 1">
                <div>
                  <span>订单号：</span>
                  {{stempInfo.orderNumber}}
                </div>
                <div>
                  <span>交易流水号：</span>
                  {{stempInfo.paymentNum}}
                </div>
                <div>
                  <span>订单交易时间：</span>
                  {{stempInfo.payTime | dateFormatYmdHm}}
                </div>
              </template>
            </div>
          ...
          <template v-if="stempInfo.divideClassesStatus == 1 && stempInfo.ifReport != 1 ">
            <div v-if="!isPhone" class="fb-box">
              您的班级信息:
              <span class="color1 fbgl" v-if="stempInfo.className != ''">{{stempInfo.className}}</span>
              <span class="color1 fbgl" v-if="stempInfo.className == ''">暂未分班</span> 您的寝室信息:
              <span class="color1">{{stempInfo.dormitoryName}}</span>
            </div>
            <div v-if="isPhone" class="fb-box">
              <p>
                您的班级信息:
                <span class="color1">{{stempInfo.className}}</span>
              </p>
              <p>
                您的寝室信息:
                <span class="color1">{{stempInfo.dormitoryName}}</span>
          ...
          <template v-if="stempInfo.ifReport == 1">
            您
            <span v-if="stempInfo.modifyTime == null">已</span>
            <span v-if="stempInfo.modifyTime != null">于</span>
            <span class="color1">{{stempInfo.modifyTime | dateFormatYmd}}</span>完成到校报到注册手续~
          </template>
          <!-- 逾期未报到 -->
          <template v-if="false">
            您
            <span class="color1">逾期未到学校报到</span>，视为自动放弃入学资格~
          </template>
        </div>
         ...
         <span>请</span><span class="color1">填写报到信息</span>并按学校通知时间<span class="color1">到校报到</span>
            ...
            <div v-else-if="stempInfo.ifSubmit == 0 && !is">
              ...
              <div class="bd-box">
                ...
                <span class="xg-nav" v-if="stempInfo.ifSubmit != 1" @click="clickModify">修改</span>
              </div>
              <preview></preview>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    //缓存预约时间
    serReserve(msg) {
      this.appointmentTime = this.getNowFormatDateFn(msg.day)+" "+msg.time;//预约的时间
      this.formData.dataTime = this.getNowFormatDateFn(msg.day);//日期
      this.formData.timeRang = msg.time;//时间
      this.appointmentTimeId = msg.id;  //预约的id
    },
    //进行预约 
    appointment(){
      const vm = this;
            let obj = {
              id: vm.stempInfo.id,
              paymentReserve: vm.appointmentTimeId
            };
      http.post("/gateway/enroll/erPayment/reserve", obj).then((xhr) => { debugger
        if (xhr.data.message=="预约成功！") {
            vm.getReserve(vm.appointmentTimeId) //查询预约
        }
    ...
    query() {
      ...
      http
        .get("/gateway/enroll/api/erRegister/admissionsProgress/" + idCard)
        .then(xhr => {
          if (xhr.code) return;
          vm.isLoading = false;
          ...
          if (vm.stempInfo.paymentDeadline) {
            vm.expireFlag =
              new Date(vm.stempInfo.paymentDeadline).getTime() +
                24 * 60 * 60 * 1000 <
              new Date().getTime();
          }
    ...
    submit() {
      ...
      if (vm.formData.dataTime != '' && vm.formData.timeRang != '') {
        ...
        this.$confirm(""+vm.appointmentTime, "确定提交预约?您预约的线下缴费时间段为:", {
        distinguishCancelAndClose: true,
        confirmButtonText: "确定",
        cancelButtonText: "取消"
      })
        .then(() => {
         vm.appointment()//确定 发送预约请求
        })
        .catch(action => {//取消
          
        });

        // vm.erRegister.id = vm.stempInfo.id;   这一块不知道在进行什么操作，先注释掉
    ...
    viewScore() {
      ...
      http
        .get("/gateway/enroll/api/erEnter/page", { params: obj })
        .then(function(xhr) {
    ...
    payBefore(type) {
      ...
      vm.orderNo =
        "JX" +
        vm.$options.filters["dateFormat"](date.getTime()) +
        date.getTime() +
        random;
      vm.amount = random / 10;
      let obj = {
    ...
    PayCost() {
      ...
      let jsonRequestData = {
        ...
        reqData: {
          ...
          signNoticeUrl:
            "http://zs.jxfls.com/gateway/enroll/erCmbPay/signNotice"
        }
      };
      http
        .post("/gateway/enroll/erCmbPay/getSignStr", jsonRequestData.reqData)
        .then(xhr => {
          ...
          form.attr(
            "action",
            window.systemParameter.CmbBank_B2B_Pay +
              "/netpayment/BaseHttp.dll?MB_EUserPay"
          );
          let input = $("<input>");
      ...
      http
        ...
        .then(xhr => {
          ...
          form.attr(
            "action",
            window.systemParameter.CmbBank_B2B_Pay +
              "/netpayment/BaseHttp.dll?PC_EUserPay"
          );
          let input = $("<input>");
          input.attr("type", "hidden");
          input.attr("name", "jsonRequestData");
    ...
    paySuccess() {
      let vm = this;
      http
        .get("/gateway/enroll/erCmbPay/singleOrder?orderNo=" + vm.orderNo)
        .then(function(xhr) {
